...
type ltfsObjects struct {
	fsObjects

	// Path to be exported over S3 API.
	fsPath string

	 // Path to metadata (another disk)
	 optionMetaPath string
...
func initMetaVolumeLTFS(fsPath, metapath, fsUUID string) error {
	// This happens for the first time, but keep this here since this
	// is the only place where it can be made less expensive
	// optimizing all other calls. Create minio meta volume,
	// if it doesn't exist yet.
	var metaBucketPath string
	if metapath != "" { //modify
	    metaBucketPath = pathJoin(metapath, minioMetaBucket) //pathJoin(fsPath, minioMetaBucket)
    } else {
        metaBucketPath = pathJoin(fsPath, minioMetaBucket)
    }

    fmt.Println(metaBucketPath)

	if err := os.MkdirAll(metaBucketPath, 0777); err != nil {
		return err
	}

	var metaTmpPath string
	if metapath != "" {
		metaTmpPath = pathJoin(metapath, minioMetaTmpBucket, fsUUID)
	} else {
	...
	var metaMultipartPath string
	if metapath != "" {
		metaMultipartPath = pathJoin(metapath, minioMetaMultipartBucket)
	} else {
...
func newLTFSObjectLayer(fsPath string, metapath string) (ObjectLayer, error) {
	...
	if err = initMetaVolumeLTFS(fsPath, metapath, fsUUID); err != nil {
		return nil, fmt.Errorf("test Unable to initialize '.minio.sys' meta volume, %s", err)
	}

	// Initialize `format.json`, this function also returns.
	var rlk *lock.RLockedFile
	if metapath != "" {
		rlk, err = initFormatFS(metapath)
	} else {
	...
	err = initBucketPolicies(fs)///bug is here
	if err != nil {
...
func (fs ltfsObjects) Shutdown() error {
	// Cleanup and delete tmp uuid.
	var removePath string
	if fs.optionMetaPath != "" {
	    removePath = pathJoin(fs.optionMetaPath, minioMetaTmpBucket, fs.fsUUID)
	} else {
		removePath = pathJoin(fs.fsPath, minioMetaTmpBucket, fs.fsUUID)
	}
	return fsRemoveAll(removePath)
}

// StorageInfo - returns underlying storage statistics.
func (fs ltfsObjects) StorageInfo() StorageInfo {
...
func (fs ltfsObjects) getBucketDir(bucket string) (string, error) {
	// !!!!!
	...
	bucketDir := ""
	if bucket == ".minio.sys" {
		bucketDir = fs.optionMetaPath + "/" + bucket
	} else {
...
func (fs ltfsObjects) statBucketDir(bucket string) (os.FileInfo, error) {
	/* !!!! statBucketDir */
...
// already exists.
func (fs ltfsObjects) MakeBucketWithLocation(bucket, location string) error {
	bucketDir, err := fs.getBucketDir(bucket)
	if err != nil {
...
func (fs ltfsObjects) GetBucketInfo(bucket string) (bi BucketInfo, e error) {
...
func (fs ltfsObjects) ListBuckets() ([]BucketInfo, error) {
...
func (fs ltfsObjects) DeleteBucket(bucket string) error {
	bucketDir, err := fs.getBucketDir(bucket)
	if err != nil {
		return toObjectErr(err, bucket)
	}
	...
	var minioMetadataBucketDir string
	if fs.optionMetaPath != "" {
	    minioMetadataBucketDir = pathJoin(fs.optionMetaPath, minioMetaBucket, bucketMetaPrefix, bucket)
	} else {
...
// update metadata.
func (fs ltfsObjects) CopyObject(srcBucket, srcObject, dstBucket, dstObject string, metadata map[string]string) (oi ObjectInfo, e error) {
	...
	if cpMetadataOnly {
		...
		if fs.optionMetaPath != "" {
			    fsMetaPath = pathJoin(fs.optionMetaPath, minioMetaBucket, bucketMetaPrefix, srcBucket, srcObject, fsMetaJSONFile)
			} else {
...
func (fs ltfsObjects) GetObject(bucket, object string, offset int64, length int64, writer io.Writer) (err error) {
	...
	if bucket != minioMetaBucket {
		var fsMetaPath string
		if fs.optionMetaPath != "" {
			    fsMetaPath = pathJoin(fs.optionMetaPath, minioMetaBucket, bucketMetaPrefix, bucket, object, fsMetaJSONFile)
			} else {
...
func (fs ltfsObjects) getObjectInfo(bucket, object string) (oi ObjectInfo, e error) {
	fsMeta := fsMetaV1{}
	var fsMetaPath string
	if fs.optionMetaPath != "" {
		fsMetaPath = pathJoin(fs.optionMetaPath, minioMetaBucket, bucketMetaPrefix, bucket, object, fsMetaJSONFile)
...
func (fs ltfsObjects) parentDirIsObject(bucket, parent string) bool {
...
func (fs ltfsObjects) PutObject(bucket string, object string, size int64, data io.Reader, metadata map[string]string, sha256sum string) (objInfo ObjectInfo, retErr error) {
	var err error
	...
	if bucket != minioMetaBucket {
		...
		if fs.optionMetaPath != "" {
			bucketMetaDir = pathJoin(fs.optionMetaPath, minioMetaBucket, bucketMetaPrefix)
		} else {
		...
		defer func() {
			...
			if retErr != nil {
				var tmpDir string
				if fs.optionMetaPath != "" {
					    tmpDir = pathJoin(fs.optionMetaPath, minioMetaTmpBucket, fs.fsUUID)
					} else {
	...
	if fs.optionMetaPath != "" {
		fsTmpObjPath = pathJoin(fs.optionMetaPath, minioMetaTmpBucket, fs.fsUUID, tempObj)
	} else {
...
func (fs ltfsObjects) DeleteObject(bucket, object string) error {
	...
	var minioMetaBucketDir string
	if fs.optionMetaPath != "" {
		minioMetaBucketDir = pathJoin(fs.optionMetaPath, minioMetaBucket)
	} else {
...
func (fs ltfsObjects) listDirFactory(isLeaf isLeafFunc) listDirFunc {
...
func (fs ltfsObjects) getObjectETag(bucket, entry string) (string, error) {
	var fsMetaPath string
	if fs.optionMetaPath != "" {
		fsMetaPath = pathJoin(fs.optionMetaPath, minioMetaBucket, bucketMetaPrefix, bucket, entry, fsMetaJSONFile)
	} else {
...
func (fs ltfsObjects) ListObjects(bucket, prefix, marker, delimiter string, maxKeys int) (loi ListObjectsInfo, e error) {
...
func (fs ltfsObjects) HealObject(bucket, object string) (int, int, error) {
	return 0, 0, traceError(NotImplemented{})
}

// HealBucket - no-op for fs, Valid only for XL.
func (fs ltfsObjects) HealBucket(bucket string) error {
	return traceError(NotImplemented{})
}

// ListObjectsHeal - list all objects to be healed. Valid only for XL
func (fs ltfsObjects) ListObjectsHeal(bucket, prefix, marker, delimiter string, maxKeys int) (loi ListObjectsInfo, e error) {
	return loi, traceError(NotImplemented{})
}

// ListBucketsHeal - list all buckets to be healed. Valid only for XL
func (fs ltfsObjects) ListBucketsHeal() ([]BucketInfo, error) {
	return []BucketInfo{}, traceError(NotImplemented{})
}

func (fs ltfsObjects) ListUploadsHeal(bucket, prefix, marker, uploadIDMarker,
