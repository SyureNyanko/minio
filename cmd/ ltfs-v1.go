...
const optionMetaPath = "/Users/syureneko/debug" 
...
func initMetaVolumeLTFS(fsPath, fsUUID string) error {
	...
	if optionMetaPath != "" { //modify
	    metaBucketPath = pathJoin(optionMetaPath, minioMetaBucket) //pathJoin(fsPath, minioMetaBucket)
    } else {
	...
	if optionMetaPath != "" {
		metaTmpPath = pathJoin(optionMetaPath, minioMetaTmpBucket, fsUUID)
	} else {
	...
	if optionMetaPath != "" {
		metaMultipartPath = pathJoin(optionMetaPath, minioMetaMultipartBucket)
	} else {
...
func newLTFSObjectLayer(fsPath string) (ObjectLayer, error) {
	if fsPath == "" {
	...
	if err = initMetaVolumeLTFS(fsPath, fsUUID); err != nil {
	...
	if optionMetaPath != "" {
		rlk, err = initFormatFS(optionMetaPath)
	} else {
...
func (fs ltfsObjects) Shutdown() error {
	...
	if optionMetaPath != "" {
	    removePath = pathJoin(optionMetaPath, minioMetaTmpBucket, fs.fsUUID)
	} else {
...
func (fs ltfsObjects) DeleteBucket(bucket string) error {
	...
	if optionMetaPath != "" {
	    minioMetadataBucketDir = pathJoin(optionMetaPath, minioMetaBucket, bucketMetaPrefix, bucket)
	} else {
...
func (fs ltfsObjects) CopyObject(srcBucket, srcObject, dstBucket, dstObject string, metadata map[string]string) (oi ObjectInfo, e error) {
	...
	if cpMetadataOnly {
		...
		if optionMetaPath != "" {
			    fsMetaPath = pathJoin(optionMetaPath, minioMetaBucket, bucketMetaPrefix, srcBucket, srcObject, fsMetaJSONFile)
			} else {
...
func (fs ltfsObjects) GetObject(bucket, object string, offset int64, length int64, writer io.Writer) (err error) {
	...
	if bucket != minioMetaBucket {
		fmt.Printf("minioMeta: %s, %s, %s, %s, %s, %s \n", fs.fsPath, minioMetaBucket, bucketMetaPrefix, bucket, object, fsMetaJSONFile)
		var fsMetaPath string
		if optionMetaPath != "" {
			    fsMetaPath = pathJoin(optionMetaPath, minioMetaBucket, bucketMetaPrefix, bucket, object, fsMetaJSONFile)
			} else {
				fsMetaPath = pathJoin(fs.fsPath, minioMetaBucket, bucketMetaPrefix, bucket, object, fsMetaJSONFile)
			}
		
		fmt.Printf("fsMetaPath %s \n", fsMetaPath)
...
func (fs ltfsObjects) PutObject(bucket string, object string, size int64, data io.Reader, metadata map[string]string, sha256sum string) (objInfo ObjectInfo, retErr error) {
	...
	if bucket != minioMetaBucket {
		...
		if optionMetaPath != "" {
			bucketMetaDir = pathJoin(optionMetaPath, minioMetaBucket, bucketMetaPrefix)
		} else {
		...
		defer func() {
			...
			if retErr != nil {
				var tmpDir string
				if optionMetaPath != "" {
					    tmpDir = pathJoin(optionMetaPath, minioMetaTmpBucket, fs.fsUUID)
					} else {
	...
	if optionMetaPath != "" {
		fsTmpObjPath = pathJoin(optionMetaPath, minioMetaTmpBucket, fs.fsUUID, tempObj)
	} else {
...
func (fs ltfsObjects) DeleteObject(bucket, object string) error {
	...
	if optionMetaPath != "" {
		minioMetaBucketDir = pathJoin(optionMetaPath, minioMetaBucket)
	} else {
...
func (fs ltfsObjects) getObjectETag(bucket, entry string) (string, error) {
	var fsMetaPath string
	if optionMetaPath != "" {
		fsMetaPath = pathJoin(optionMetaPath, minioMetaBucket, bucketMetaPrefix, bucket, entry, fsMetaJSONFile)
	} else {
